  ...
  return (
    <Box sx={{
      ...
      <Container maxWidth="md" sx={{ mb: resultsRef.current ? 8 : 0 }}>
        <Box ref={resultsRef}>
          {(result || blacklistResult) && (() => {
            // Xác định 3 trạng thái logic
            const isDangerous = !blacklistResult?.isSafe || result?.safe === false
            const isSafe = blacklistResult?.isSafe && result?.safe === true
            const isWarning = blacklistResult?.isSafe && result?.safe === null

            // Cấu hình giao diện dựa trên trạng thái (Khôi phục lại kích thước to)
            let statusConfig = {
              bg: 'linear-gradient(135deg, rgba(16, 185, 129, 0.15) 0%, rgba(6, 182, 212, 0.15) 100%)',
              border: 'rgba(16, 185, 129, 0.4)',
              line: 'linear-gradient(90deg, #10b981, #06b6d4, #10b981)',
              icon: <CheckCircleIcon sx={{ fontSize: 72, color: '#10b981', filter: 'drop-shadow(0 0 20px rgba(16,185,129,0.6))' }} />,
              title: 'AN TOÀN TUYỆT ĐỐI',
              titleGradient: 'linear-gradient(135deg, #10b981 0%, #06b6d4 100%)',
              message: blacklistResult?.message || result?.message,
            ...
            if (isDangerous) {
              statusConfig = {
                ...
                line: 'linear-gradient(90deg, #ef4444, #f59e0b, #ef4444)',
                icon: <WarningIcon sx={{ fontSize: 72, color: '#ef4444', filter: 'drop-shadow(0 0 20px rgba(239,68,68,0.6))' }} />,
                title: 'PHÁT HIỆN NGUY HIỂM!',
                titleGradient: 'linear-gradient(135deg, #ef4444 0%, #f59e0b 100%)',
                message: blacklistResult?.isSafe ? result?.message : blacklistResult?.message,
              ...
              statusConfig = {
                ...
                line: 'linear-gradient(90deg, #f59e0b, #fbbf24, #f59e0b)',
                icon: <ErrorIcon sx={{ fontSize: 72, color: '#f59e0b', filter: 'drop-shadow(0 0 20px rgba(245,158,11,0.6))' }} />,
                title: 'CHƯA CÓ DỮ LIỆU',
                titleGradient: 'linear-gradient(135deg, #f59e0b 0%, #fbbf24 100%)',
                message: 'Url chưa có trong cơ sở dữ liệu, hãy cẩn thận khi truy cập.',
            ...
            return (
              <Fade in timeout={800}>
                <Paper elevation={0} sx={{
                  ...
                  mb: 6,
                  boxShadow: `0 20px 60px -10px ${statusConfig.border}`,
                  position: 'relative',
                  overflow: 'hidden',
                  '&::before': {
                    content: '""',
                    position: 'absolute', top: 0, left: 0, right: 0, height: '4px',
                    background: statusConfig.line,
                    backgroundSize: '200% 100%',
                    animation: 'gradient 3s ease infinite'
                  }
                }}>
                  {/* 1. Header Trạng thái (VARIANT H3 - TO) */}
                  <Box sx={{ textAlign: 'center', mb: 5 }}>
                    <Box sx={{
                      display: 'inline-flex', p: 3, borderRadius: '50%',
                      bgcolor: statusConfig.iconBg, mb: 3,
                      animation: isSafe ? 'pulse 2s ease-in-out infinite' : isDangerous ? 'shake 0.5s ease-in-out' : 'none'
                    }}>
                      {statusConfig.icon}
                    </Box>

                    {isSafe && result?.trusted && (
                      <Box sx={{ mb: 3 }}>
                        <Chip
                          ...
                          sx={{
                            ...
                            px: 3, py: 3, height: 'auto', borderRadius: 3,
                            boxShadow: '0 8px 20px -4px rgba(16, 185, 129, 0.5)',
                    ...
                    <Typography variant="h3" sx={{
                      fontWeight: 900, mb: 2, letterSpacing: -1,
                      background: statusConfig.titleGradient,
                      WebkitBackgroundClip: 'text',
                      WebkitTextFillColor: 'transparent',
                      textShadow: 'none'
                    }}>
                    ...
                    <Typography variant="h6" sx={{ color: '#cbd5e1', fontWeight: 500, maxWidth: '600px', mx: 'auto', lineHeight: 1.6 }}>
                  ...
                  <Grid container spacing={3}>
                    ...
                    <Grid item xs={12} md={6}>
                      <Card elevation={0} sx={{
                        ...
                        '&:hover': { transform: 'translateY(-4px)', boxShadow: `0 12px 30px -8px ${statusConfig.detailColor}4d`, borderColor: `${statusConfig.detailColor}66` }
                      }}>
                        <CardContent sx={{ p: 4 }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
                            <Box sx={{ p: 1.5, borderRadius: 2, background: 'linear-gradient(135deg, #06b6d4 0%, #0891b2 100%)', display: 'flex' }}>
                              <ShieldIcon sx={{ color: '#fff', fontSize: 28 }} />
                            </Box>
                            <Typography variant="h6" sx={{ color: '#06b6d4', fontWeight: 800, letterSpacing: 0.5, textTransform: 'uppercase' }}>
                        Cơ sở dữ liệu
                            </Typography>
                          </Box>
                          ...
                          <Box sx={{
                            ...
                            {/* VARIANT H4 (TO NHƯ CŨ) */}
                            <Typography variant="h4" sx={{
                              fontWeight: 900,
                              color: blacklistResult?.isSafe ? '#10b981' : '#ef4444',
                              mb: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 1
                            }}>
                              {blacklistResult?.isSafe ? <><CheckCircleIcon sx={{ fontSize: 32 }} /> Sạch</> : <><ErrorIcon sx={{ fontSize: 32 }} /> Nguy hiểm!</>}
                            </Typography>
                            {!blacklistResult?.isSafe && blacklistResult?.data && (
                              <Box sx={{ mt: 2 }}>
                                <Chip label={blacklistResult.data.scamType || 'N/A'} color="error" sx={{ fontWeight: 700, fontSize: '0.85rem', mb: 1 }} />
                    ...
                    <Grid item xs={12} md={6}>
                      <Card elevation={0} sx={{
                        ...
                        border: '1px solid rgba(255,255,255,0.1)',
                        borderRadius: 4, transition: 'all 0.3s ease',
                        '&:hover': { transform: 'translateY(-4px)', boxShadow: '0 12px 30px -8px #8b5cf64d', borderColor: 'rgba(139, 92, 246, 0.4)' }
                      }}>
                        <CardContent sx={{ p: 4 }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
                            <Box sx={{ p: 1.5, borderRadius: 2, background: 'linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%)', display: 'flex' }}>
                              <VisibilityIcon sx={{ color: '#fff', fontSize: 28 }} />
                            </Box>
                            <Box>
                              <Typography variant="h6" sx={{ color: '#8b5cf6', fontWeight: 800, letterSpacing: 0.5, textTransform: 'uppercase' }}>Phân tích bảo mật</Typography>
                              <Typography variant="caption" sx={{ color: '#a78bfa', fontSize: '0.75rem', display: 'block' }}>VirusTotal & Blacklist Database</Typography>
                            </Box>
                          </Box>

                          {result?.aiAnalysis?.success ? (
                            <Box>
                              <Box sx={{ p: 3, borderRadius: 3, background: 'linear-gradient(135deg, rgba(139, 92, 246, 0.1) 0%, rgba(124, 58, 237, 0.1) 100%)', border: '2px solid rgba(139, 92, 246, 0.3)', textAlign: 'center', mb: 2 }}>
                                <Typography variant="caption" sx={{ color: '#a78bfa', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1 }}>Điểm tin cậy</Typography>
                                <Typography variant="h2" sx={{ color: '#8b5cf6', fontWeight: 900, my: 1, textShadow: '0 0 20px rgba(139, 92, 246, 0.3)' }}>{result.aiAnalysis.trustScore || 0}/100</Typography>
                                <Chip label={result.aiAnalysis.riskLevel?.toUpperCase() || 'UNKNOWN'} size="small" sx={{ bgcolor: '#8b5cf6', color: '#fff', fontWeight: 700 }} />
                              </Box>
                              <Box sx={{ p: 2.5, borderRadius: 3, bgcolor: 'rgba(139, 92, 246, 0.05)', border: '1px solid rgba(139, 92, 246, 0.2)' }}>
                                <Typography variant="body2" sx={{ color: '#e2e8f0', lineHeight: 1.6 }}>{result.aiAnalysis.analysis}</Typography>
                              </Box>
                            </Box>
                          ) : (
                            <Grid container spacing={2}>
                              {/* KHỐI NÀY ĐÃ ĐƯỢC KHÔI PHỤC VARIANT H3 VÀ PADDING 2.5 (TO NHƯ CŨ) */}
                              <Grid item xs={6}>
                                <Box sx={{ p: 2.5, borderRadius: 3, bgcolor: 'rgba(239, 68, 68, 0.1)', border: '2px solid rgba(239, 68, 68, 0.2)', textAlign: 'center', transition: 'all 0.3s', '&:hover': { borderColor: 'rgba(239, 68, 68, 0.5)', transform: 'scale(1.05)' } }}>
                                  <ErrorIcon sx={{ color: '#ef4444', fontSize: 28, mb: 1 }} />
                                  <Typography variant="h3" sx={{ color: '#ef4444', fontWeight: 900, mb: 0.5, textShadow: '0 0 20px rgba(239, 68, 68, 0.3)' }}>{result?.details?.malicious || 0}</Typography>
                                  <Typography variant="caption" sx={{ color: '#cbd5e1', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1 }}>Độc hại</Typography>
                                </Box>
                              </Grid>
                              <Grid item xs={6}>
                                <Box sx={{ p: 2.5, borderRadius: 3, bgcolor: 'rgba(245, 158, 11, 0.1)', border: '2px solid rgba(245, 158, 11, 0.2)', textAlign: 'center', transition: 'all 0.3s', '&:hover': { borderColor: 'rgba(245, 158, 11, 0.5)', transform: 'scale(1.05)' } }}>
                                  <WarningIcon sx={{ color: '#f59e0b', fontSize: 28, mb: 1 }} />
                                  <Typography variant="h3" sx={{ color: '#f59e0b', fontWeight: 900, mb: 0.5, textShadow: '0 0 20px rgba(245, 158, 11, 0.3)' }}>{result?.details?.suspicious || 0}</Typography>
                                  <Typography variant="caption" sx={{ color: '#cbd5e1', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1 }}>Đáng ngờ</Typography>
                                </Box>
                              </Grid>
                            </Grid>
                          )}
